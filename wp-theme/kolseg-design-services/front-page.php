...
<main>
  <?php
  $kolseg_force_home_fallback = is_home();
  $kolseg_home_fallback_content = kolseg_get_seed_source_content('home');
  ?>
  <?php if (!$kolseg_force_home_fallback && have_posts()) : while (have_posts()) : the_post(); ?>
    ...
    <?php endif; ?>
  <?php endwhile; elseif (!empty($kolseg_home_fallback_content)) : ?>
    <?php echo $kolseg_home_fallback_content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
  <?php else : ?>
    <section class="hero hero-home hero-home-clean">
      <div class="hero-media">
        <img src="<?php echo esc_url(kolseg_get_theme_image('kolseg_hero_bg', 'live-studio-main.jpg')); ?>" alt="KOLSEG live studio setup">
